...
public class pochodna {
		...
		public static abstract class Operator extends Atom {
			...
			public String toString() {
				...
				return this.left.toString() + " " + this.sign() + " " + String.valueOf(this.right.count() - 1)+ this.right.toString().slice(4);	
				}
		...
		public static class Power extends Operator{
			...
			public Double count(){
				return this.right.count() * Math.pow(this.left.count(),this.right.count()-1.0);
			}
			protected String sign(){
				return "^";
			}
