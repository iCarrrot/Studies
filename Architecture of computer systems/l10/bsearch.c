...
//czas bez bin search: 9.13s, heap: 0.33s.
...
bool heap_search(int *arr, int size, int x) {
  ...
  while(i < size){
    if (arr[i] == x ) return true;
     ///zamieniłem część zakomentowaną na kolejne dwie linijki i uzyskałem czas ~13 razy lepszy (program pierwotnie trwał 5.11s)
    /* if (arr[i] > x) i = (i << 1) +1;
    else i = (i << 1) + 2;   */
    temp=(arr[i] > x);
    i=(i<<2)+2-temp;
